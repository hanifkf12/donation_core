...
impl Command<UserService, (), Option<Vec<User>>> for GetUsersUseCase {
    async fn execute(&self, service: &UserService, pool: &PgPool, _input: ()) -> Result<Option<Vec<User>>, Error> {
...
impl Command<UserService, Uuid, Option<User>> for GetUserByIdUseCase {
    async fn execute(&self, service: &UserService, pool: &PgPool, id: Uuid) -> Result<Option<User>, Error> {
